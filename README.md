...
The `data/` directory in this repo holds the tokenized `train.bin`/`val.bin` and
`corpus_stats.json`. The cleaned JSONL shards are regenerated by the command
above; a single sample shard is kept under `data/sample/` for inspection.
