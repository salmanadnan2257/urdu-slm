...
$\sim$250 docs/s, extrapolating to over two hours. The rewrite hashes all of a document's
shingles with xxhash and computes the whole signature in two vectorized numpy operations,
reaching $\sim$2400 docs/s.
...
data. Its position corroborates the README's account: the main phase was $\sim$2.4B tokens,
which at 524,288 tokens/step lands at step $\sim$4,578, and the log's jump is between 4,560
and 4,580.}
\end{figure}
...
from 0.9 to 20.2, right at the Chinchilla-optimal $\sim$20. Perplexity fell from 38.02 to
28.93, a genuine $\sim$24\% reduction. Cloze accuracy went from 34.9\% to 39.5\%.
...
correct continuation is a verbatim copy of text $\sim$70 tokens earlier, which rewards copying
...
and $\sim$105,000 tok/s: the estimate was low by a factor of $\sim$3.3, and every cost projection
...
\item \textbf{Stale docs}: \file{evaldata/README.md} still says 45 items,
      \file{tiny.yaml} still says "\textasciitilde{}10M params", \file{base.yaml} still says "24GB GPU",
...
re-downloading $\sim$1.3GB of sources and hours of pipeline. What \emph{was} verified: the
...
\item[Annealing] Finishing training on a small, high-quality data slice after a large
  ...
  with a $\sim$100M-token Wikipedia-only phase; the switch is visible as a loss step in the log.
...
\item[Chinchilla-optimal] The rule of thumb that compute-optimal training uses $\sim$20 tokens
