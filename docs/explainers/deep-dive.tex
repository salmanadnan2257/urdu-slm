...
\begin{tikzpicture}[node distance=8mm]
  ...
  \node[extbox,text width=5.0cm,anchor=north west] at ($(doc.south west)+(0,-1.6)$)
    {\scriptsize The two boxed numpy operations replace\\ datasketch's per-shingle Python loop.\\ Measured: ~250 docs/s to ~2400 docs/s.};
\end{tikzpicture}}
...
\begin{axis}[
  ...
  legend style={font=\footnotesize, at={(0.98,0.06)}, anchor=south east, draw=linegrey},
...
The \code{tiny} preset's comment in \file{configs/tiny.yaml} still says "~10M params",
...
\begin{tikzpicture}[node distance=6mm]
  \node[store,text width=2.4cm] (in) {token ids\\ \scriptsize (B, T) int64};
  \node[box,right=9mm of in,text width=2.5cm] (emb) {\code{tok\_emb}\\ \scriptsize Embedding\\ 32000 x d\_model};
  \node[box,right=8mm of emb,text width=1.7cm] (drop) {\code{drop}\\ \scriptsize Dropout};

  % the block
  \node[box,right=10mm of drop,text width=2.2cm,fill=softgrey] (n1) {\code{attn\_norm}\\ \scriptsize RMSNorm};
  \node[box,right=7mm of n1,text width=2.6cm] (att) {\code{attn}\\ \scriptsize RoPE causal\\ self-attention};
  \node[dec,right=7mm of att,text width=0.6cm] (add1) {$+$};
  \node[box,right=8mm of add1,text width=2.2cm,fill=softgrey] (n2) {\code{mlp\_norm}\\ \scriptsize RMSNorm};
  \node[box,right=7mm of n2,text width=2.0cm] (mlp) {\code{mlp}\\ \scriptsize SwiGLU};
  \node[dec,right=7mm of mlp,text width=0.6cm] (add2) {$+$};

  \node[box,right=9mm of add2,text width=2.2cm,fill=softgrey] (fn) {\code{final\_norm}\\ \scriptsize RMSNorm};
  \node[box,right=7mm of fn,text width=2.3cm] (head) {\code{lm\_head}\\ \scriptsize Linear, no bias};
  \node[store,right=8mm of head,text width=2.5cm] (out) {logits\\ \scriptsize (B, T, 32000)};
  ...
  % residual skips
  \draw[dflow] (drop.north) to[out=60,in=120] (add1.north);
  \draw[dflow] (add1.north) to[out=60,in=120] (add2.north);

  % rope feed
  \node[extbox,below=13mm of att,text width=2.9cm] (rope) {\code{\_rope(T,...)}\\ \scriptsize cos/sin cache};
  \draw[dflow] (rope) -- (att);

  % tied weights
  \draw[dflow] (emb.south) to[out=-90,in=-90,looseness=0.35] node[lbl,below] {tied: one weight matrix, used twice} (head.south);

  \begin{scope}[on background layer]
    \node[draw=accent,dashed,rounded corners=3pt,inner sep=7pt,
          fit=(n1)(att)(add1)(n2)(mlp)(add2)] (blk) {};
  \end{scope}
  \node[lbl,above=1mm of blk.north] {\code{Block}, repeated \code{n\_layer} times (14 for base)};
\end{tikzpicture}}
...
the measured ~105,000 tokens/second in Section~\ref{sec:runs}.
\end{jargon}
...
\item \textbf{No \code{DataLoader}, no prefetch thread.} Batches are built synchronously
      ...
      device copy. At the measured ~105,000 tokens/second this evidently was not the
...
comment explains that as "~4 epochs of the 111.7M-token corpus". Both are v1-era facts.
...
\file{configs/tiny.yaml}'s header says "~10M params"; the real figure is 13.01M, measured
above. \file{configs/base.yaml}'s \code{grad\_accum} comment says "effective ~524k
...
Measured throughput & ~99,470 tokens/second sustained \\
...
\node[lbl, anchor=south east, text=warnred] at (axis cs:4550,6.6)
  {anneal starts: data source\\ switches to Wikipedia-only};
\legend{train loss}
\end{axis}
\end{tikzpicture}
...
GPU time and cost & ~75 min, ~\$1.90 & 6h 39m, ~\$13.06 \\
\bottomrule
\end{tabular}
...
error: it projects the 450M-token run at ~4.2 hours for about \$6, where v1 actually did
...
are named rather than buried: throughput is scaled from v2's measured ~105,000 tok/s by
the parameter ratio ($336/124 \approx 2.71$, projecting ~38,700 tok/s), which assumes
...
smoke-test a few hundred steps before committing the ~\$40-45 budget is the right one.
...
\item \textbf{Upsampling repeats documents back-to-back inside a single context window}
      ...
      Wikipedia paragraph down contiguously, and at ~70 tokens per paragraph all three
...
\item \textbf{Stale config comments}: \file{tiny.yaml} says "~10M params" (real: 13.01M);
...
v1: 858 steps, 449.8M tokens, ~99,470 tok/s & \textbf{verified} &
    read \file{runs/base/metrics.jsonl} \\
v2: 4,768 steps, 2,499,805,184 tokens, ~105,000 tok/s, loss to ~2.74 &
