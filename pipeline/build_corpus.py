...
from .sources import read_wiki_dump, read_leipzig_tar, read_plain_text, read_wikidata_facts
...
def stage_extract(args, work):
    ...
    for spec in args.plain:
        ...
        run_source(name, read_plain_text(Path(args.raw_dir) / fname, name,
                                         max_docs=args.limit_docs or None))
    if args.wikidata:
        run_source("wikidata", read_wikidata_facts(Path(args.raw_dir) / args.wikidata,
                                                    max_docs=args.limit_docs or None))
...
def main():
    ...
    ap.add_argument("--wikidata", default=None,
                    help="wikidata_facts.jsonl filename in raw-dir (see pipeline.fetch_wikidata)")
